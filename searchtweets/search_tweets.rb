...
class SearchTweets
  ...
  attr_accessor :tweets, #An array of Tweet JSON objects.
                ...
                :until_id,
                :max_results,
                #:format, #Labs v1 support.
                :tweet_fields, #Labs v2 support.
  ...
  def get_system_config(config_file)
    ...
    @tweet_fields = config['options']['tweet.fields']
  ...
  def build_data_request(query, start_time = nil, end_time = nil, since_id = nil, until_id = nil, max_results = nil, tweet_fields = nil, next_token = nil)
    ...
    if !tweet_fields.nil?
      request['tweet.fields'] = tweet_fields
    else
      request['tweet.fields'] = @tweet_fields
    end
  ...
  def make_data_request(query, start_time, end_time, since_id, until_id, max_results, tweet_fields, next_token)

    result_count = nil #Only Labs returns this.

    @requester.url = @urlData

    request_data = build_data_request(query, start_time, end_time, since_id, until_id, max_results, tweet_fields, next_token)
  ...
  def get_data(query, start_time, end_time, since_id, until_id)
    ...
    loop do
      ...
      response_metadata = make_data_request(query, start_time, end_time, since_id, until_id, @max_results, @tweet_fields, response_metadata['next_token'])
    ...
    return nil, @newest_id_study if @write_mode == 'standard_out' #If sending to standard out, then no data to return.
    return @tweets, @newest_id_study if @write_mode == 'hash'
    return @tweets.to_json, @newest_id_study if @write_mode == 'json'
  end #get_data
